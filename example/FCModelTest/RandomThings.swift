...
class RandomThings : NSObject {
    class func randomName() -> String {
        return randomNames[Int(self.randomUInt32()) % count(randomNames)]
    }
    
    class func randomUInt32() -> UInt32 {
        let p = UnsafeMutablePointer<UInt8>.alloc(4)
        SecRandomCopyBytes(kSecRandomDefault, 4, p) // gotta have priorities. Marco's not going to put an insecure RNG in his throwaway random-stuff class.
        return UInt32(p.memory)
    }
}